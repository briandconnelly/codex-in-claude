...
- **`model` is reserved for the first-class controls** (#310). `meta.model` (and
  ...
  envelope reports the per-call/server value (null in the common case), so every spelling of the
  key is refused at parse time. Set `CODEX_IN_CLAUDE_MODEL` or the per-call parameter instead —
  those flow into `resolved_defaults` and `meta.model` correctly. This is an exact-key
  reservation: `model_provider`/`model_providers.*` (this knob's motivating use case) and other
  `model_*` keys still pass through, and `model_reasoning_effort` stays allowed until the
  reasoning-effort surface (#309) lands its first-class replacement and reserves it. An opaque
