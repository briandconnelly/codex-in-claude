...
def test_status_keeps_the_support_verdict_on_the_raw_probe_output(monkeypatch, clean_env):
    """Identity stays raw. `0.<BEL>149.1` DISPLAYS as a plausible supported version, but the
    raw string does not parse — so the verdict must be null, never repaired into true."""
    monkeypatch.setattr(server.codex, "codex_version", lambda: "codex-cli 0.\x07149.1")
    monkeypatch.setattr(server.codex, "login_status", lambda: (False, None))
    res = server.codex_status()
    assert res["codex_version"] == "codex-cli 0.149.1"
...
async def test_status_reports_reasoning_effort_defaults(monkeypatch, clean_env):
    ...
    monkeypatch.setattr(server.codex, "codex_version", lambda *a, **k: "codex-cli 0.149.1")
