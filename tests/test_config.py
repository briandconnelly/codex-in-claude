...
@pytest.mark.parametrize(
    "version,expected",
    [("codex-cli 0.147.0", True), ("codex-cli 0.999.0", False), ("garbage", None), (None, None)],
)
...
def test_supported_versions_env_override(clean_env):
    ...
    assert config.version_supported("codex-cli 0.147.0") is False


def test_supported_versions_bad_env_falls_back(clean_env):
    clean_env.setenv("CODEX_IN_CLAUDE_SUPPORTED_VERSIONS", "garbage")
    assert config.version_supported("codex-cli 0.147.0") is True
