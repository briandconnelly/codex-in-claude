...
Adopted from moonbridge via the shared pontonier test kit. Description-only
...
next to the facts that justify them; the pontonier conformance check also holds
the declarative `PONTONIER_CONTRACT` to its own bans and pins its derivations
...
def test_contract_instance_derives_from_legacy_constants():
    """The declarative PONTONIER_CONTRACT and the constants codex.py still consumes
...
def test_forbidden_phrases_are_still_justified():
    ...
    assert cli_contract.PONTONIER_CONTRACT.backend_id == "codex"
