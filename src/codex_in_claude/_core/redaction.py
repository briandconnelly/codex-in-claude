...
SECRET_VALUE_PATTERNS = [
    ...
    # matcher, so no per-pattern tweak reaches it. Nothing is lost by running them early —
    # a userinfo match either contains a later candidate (which its replacement covers) or
    # does not overlap one at all, and neither run's class admits `:` or `@`, so a match can
    # never straddle the boundary of a span one of these already replaced.
    #
