...
_DEVELOPER_INSTRUCTIONS_FULL = (
    ...
    "mechanical. The verdict clause is one instruction inside a broader limit: "
    "compliance with the caller-supplied text is BEST-EFFORT. Codex may follow it in "
    "full, in part, or not at all, and no dedicated result field attests which "
    "happened — meta's fingerprint attests the request this server accepted and staged "
    "for the run, never what the model did with it. Non-compliance "
    "may be silent, though not always: this server's framing instructs Codex to say so "
    'when the text conflicts with the rules above it, and detail="full" returns '
    "`raw_response.text` (the closest-to-source carrier, after best-effort redaction). "
    "So an ignored stance can produce output indistinguishable from a "
    "run that carried no caller text — while meta still tells the two REQUESTS apart. "
    "Treat it as a request for focus, tone, or emphasis, not a contract. "
    "Normalized once (stripped; blank means omitted); refused pre-spend when "
...
PARAMETER_CONTRACTS: dict[str, ParamContract] = {
    ...
    "developer_instructions": ParamContract(
        ...
        summary=(
            ...
            "is instructed, not compelled — verdicts stay its own, and compliance with the "
            "rest is best-effort and may be silent. Rides the codex "
