...
- The [`codex` CLI](https://developers.openai.com/codex/cli) on `PATH`, authenticated
  (`codex login` — ChatGPT or API key). Tested against `codex-cli 0.152`; the supported range lives
...
- Both consult and review (sync and `_async`) also take `developer_instructions` — optional caller
...
| `CODEX_IN_CLAUDE_EXTRA_ARGS` | unset | extra global `codex` options added to every **paid** exec call (consult/review/delegate), so you can select a `model_provider`/`--profile` even under `ignore-config` isolation (which drops `config.toml`, leaving `-c` the only lever). Allowlist only: `-c`/`--config KEY=VALUE`, `-p`/`--profile NAME`, `--enable`/`--disable FEATURE`. The plugin-owned features (`remote_plugin`, `sleep_tool`) are refused in every spelling — `--enable`/`--disable` and `-c features.…` — see `COMPATIBILITY.md` → "Operator extra-args passthrough" for the full deny list. Anything else is refused with `extra_args_rejected` **before any spend** — including `-c` keys under `sandbox`/`approval_policy`/`shell_environment_policy` (guarantee-weakening), the reserved `model`/`model_reasoning_effort` keys (use `CODEX_IN_CLAUDE_MODEL`/`CODEX_IN_CLAUDE_REASONING_EFFORT` or the per-call `model`/`reasoning_effort` parameters instead), and the instruction-bearing `developer_instructions`/`model_instructions_file`/`experimental_instructions_file`/`instructions`/`model_catalog_json` keys (they would place operator prose above the server's framing with no record in `meta`; for `developer_instructions` use the per-call, meta-reported parameter on consult/review instead — #556; the other four have no first-class control on purpose) — plus their case/quote lookalikes (deny rules and rationale in `COMPATIBILITY.md`). `-c` values may hold secrets, so they are never echoed in `codex_status` or errors. A `--profile` layers an on-disk TOML this server cannot inspect — an operator-trust boundary (see `COMPATIBILITY.md`) |
| `CODEX_IN_CLAUDE_MAX_INPUT_BYTES` | 200000 | byte cap on author input: gathered diffs are truncated to it, author text above it is rejected with `input_too_large` (consult counts `question`+`extra_context`+`developer_instructions` together; review sums `extra_context`+`developer_instructions` pre-spend and bounds the diff separately; delegate caps each input separately) |
