...
- Supplied prompts and context (`question`, `task`, `extra_context`, and similar author input) are
  ...
  them (for delegate, the committed ones auto-load in the throwaway worktree). The plugin's
  isolation flags do not suppress this project-level context. Best-effort redaction protects
