...
Optionally call `codex_dry_run` first (free) to preview the scope and diff size. That
preview covers the input the plugin assembles; it does not invoke Codex, so it neither
enumerates nor bounds the files the model itself reads and sends during the paid run.
