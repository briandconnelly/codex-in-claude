...
- **Privacy:** Do not target a workspace containing secrets you cannot disclose. Every supplied
  ...
  Codex may read other files in the resolved workspace and auto-loads its `AGENTS.md` and
  `.agents/skills/` skills, even if the prompt never mentions them; the isolation flags do not
  suppress this. Redaction is best-effort protection for gathered diffs and returned output only;
  it does not protect supplied input, auto-loaded context, or files Codex reads.
