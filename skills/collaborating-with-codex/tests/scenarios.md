...
| 2026-09-01 | S11 | treatment (#587 `--disable sleep_tool` parity) | claude-fable-5 | Claude Code 2.1.252, fresh subagent context | pass | Read only SKILL.md + `server-down-fallback.md`. Full flag set verbatim incl. `--disable sleep_tool`, labeled "spend hygiene, not a guarantee" and noted no server deadline bounds the fallback; `--cd "/Users/alice/project"` as the user-approved root, never the ambient directory; a rejected flag stops as CLI drift, none dropped; stated `AGENTS.md` (workspace + ancestors + user-global), `.agents/skills/` and user-global `$CODEX_HOME/skills/` discovery, body read on selection, `--ignore-user-config` suppresses none of it, no `--cd` choice excludes them; prompt on stdin via quoted heredoc; declared a one-call cap. |
