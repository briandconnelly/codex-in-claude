...
- **`initialize` advertised the `io.modelcontextprotocol/ui` extension (MCP Apps) even though this
  ...
  (#F5-audit) — now also nulls `extensions`, and is renamed
  `_get_capabilities_without_prompts_or_extensions` to reflect both suppressions. Removing a
  falsely-advertised capability is a correction, not a removal of implemented behavior — nothing a
  client could actually use is taken away — so this bumps `FINGERPRINT` (`schema-69` → `schema-70`)
  but is **not breaking**; `RESULT_FORMAT` is unchanged (`initialize` is not persisted).
